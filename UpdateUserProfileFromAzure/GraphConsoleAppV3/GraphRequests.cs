...
namespace GraphConsoleAppV3
{
    ...
    {
        const string AdminTargetWebUrl = "https://xxx-admin.sharepoint.com/";
        const string SPTargetWebUrl = "https://xxx.sharepoint.com/";
        const string GroupListName = "GrupperTest";

        const string UserName = "xxx@xxx.onmicrosoft.com";
        // GetInput("Enter your user name:", false, defaultForeground);

        const string PwdS = ""; // GetInput("Enter your password:", true, defaultForeground);
        const string MySiteUrl = "https://xxx-my.sharepoint.com/";
        ...
        public static async Task AppMode()
        {
            ...
            //await PrintAllExtensions(client);


            //await GetAllUsers(client);
            await SPGetGroupsFrom(client);

            //var apps = await client.Applications.ExecuteAsync();
            //var app = apps.CurrentPage[0] as Application;
            //await CreateSchemaExtensions(client, app);
            Console.WriteLine("Press a key yo'all!");

            Console.ReadKey();

        }

        private static async Task PrintAllExtensions(ActiveDirectoryClient client)
        {
            ...
            {
                Console.WriteLine("\nExtension: {0} {1} {2}", ext.Name, ext.AppDisplayName, ext.TargetObjects[0]);
            }
        }

        #region operations with users

        private static async Task GetAllUsers(IActiveDirectoryClient client)
        {

            Console.WriteLine("Gettin' all users yo'all!");
            //*********************************************************************
            // People picker
            ...
            try
            {
                ...
                searchResults = await userCollection.Where(user =>
                        user.UserType.Equals("Member")).ExecuteAsync(); //&& user.Mail=="kh@bahrda.onmicrosoft.com"
                usersList = searchResults.CurrentPage.ToList();
            }
            catch (Exception e)
            {
                Program.WriteError("\nError getting Users {0}", Program.ExtractErrorMessage(e));
            }

            if (usersList != null && usersList.Count > 0)
            {
                using (var ctx = new ClientContext(AdminTargetWebUrl))
                {
                    ctx.Credentials = new SharePointOnlineCredentials(UserName, _pwd);
                    ...
                    do
                    {
                        ...
                        {
                            var userPhotoUrl = "";
                            Console.WriteLine("User {0} DisplayName: {1}  UPN: {2} Mobile:{3} Photo{4}",
                                index, user.DisplayName, user.UserPrincipalName, user.Mobile, "");

                            index++;
                            try
                            {
                                IStreamFetcher photo = (IStreamFetcher)user.ThumbnailPhoto;
                                try
                                {
                                    DataServiceStreamResponse response =
                                        photo.DownloadAsync().Result;
                                    string newImageNamePrefix = user.UserPrincipalName.Replace("@", "_")
                                        .Replace(".", "_");
                                    userPhotoUrl = UploadUserPhoto.UploadImageToSpo(newImageNamePrefix, response.Stream);

                                    Console.WriteLine("\nUser {0} GOT thumbnailphoto", user.DisplayName);
                                }
                                catch (Exception e)
                                {
                                    Console.WriteLine("\nError getting the user's photo - may not exist {0} {1}",
                                        e.Message,
                            ...
                            UpdateSpUserProfile(user, peopleManager, userPhotoUrl);
                        }
        ...
        //, System.Collections.Generic.List<KeyValuePair<string,object>> extProperties)
        {

            var ctx = peopleManager.Context;
            var date = "";
            var accountName = "i:0#.f|membership|" + user.UserPrincipalName;
            var personProperties = peopleManager.GetPropertiesFor(accountName);
            ctx.Load(personProperties);
            ctx.ExecuteQuery();

            var usr = (User)user;
            var userExtProperties = usr.GetExtendedProperties();

            Console.WriteLine("Updating: " + accountName);

            foreach (var userExtProperty in userExtProperties)
            {
                //Console.WriteLine(userExtProperty.Key + ": " + userExtProperty.Value);
                if (userExtProperty.Key == "extension_792b7e3d6a254f3b9843b93f0811504c_extensionAttribute3")
                {
                    IFormatProvider culture = new System.Globalization.CultureInfo("nb-No", true);

                    date = userExtProperty.Value.ToString();
                    DateTime dt2 = DateTime.Parse(userExtProperty.Value.ToString(), culture,
                        System.Globalization.DateTimeStyles.AssumeLocal);
                    Console.WriteLine(dt2.ToString("yyyy-MM-dd"));
                    if (userExtProperty.Value.ToString().Length == 10)
                    {

                        peopleManager.SetSingleValueProfileProperty(accountName, "Ansattdato",
                            dt2.ToString("yyyy-MM-dd"));
                        //Console.WriteLine(userExtProperty.Value.ToString());
                    }
            ...
            peopleManager.SetSingleValueProfileProperty(accountName, "CellPhone", user.Mobile);
            peopleManager.SetSingleValueProfileProperty(accountName, "PictureURL", userPhotoUrl);
            peopleManager.SetSingleValueProfileProperty(accountName, "SPS-PicturePlaceholderState", "0");

            //Console.WriteLine("Updated: " + accountName);
            try
            {
            ...
            {
                Console.WriteLine("Error: " + accountName + " " + date);
            }
        ...
        static List<Dictionary<string, int[]>> _groupsAndMembersList;

        private static async Task SPGetGroupsFrom(ActiveDirectoryClient client)
        {
            using (var ctx = new ClientContext(SPTargetWebUrl))
            {
                ctx.Credentials = new SharePointOnlineCredentials(UserName, _pwd);
                ctx.RequestTimeout = Timeout.Infinite;
                var list = ctx.Web.GetListByTitle(GroupListName);

                var groups = list.GetItems(CamlQuery.CreateAllItemsQuery(400));
                ctx.Load(groups);
                ctx.ExecuteQueryRetry();
                foreach (var group in groups)
                {
                    //Hovedgruppe
                    _groupsAndMembersList = new List<Dictionary<string, int[]>>();

                    await PrintGroupMembers(client, group["Title"].ToString(), group.Id, ctx, "Hovedgruppe");

                    SPUpdateGroupsList(group.Id, group["Title"].ToString(), _groupsAndMembersList, ctx);
                }

            }
        }

        private static void SPUpdateGroupsList(int groupId, string groupName, List<Dictionary<string, int[]>> groupsAndMembersList,
            ClientContext ctx)
        {
            var list = ctx.Web.GetListByTitle(GroupListName);
            var item = list.GetItemById(groupId);
            ctx.Load(item);
            ctx.ExecuteQueryRetry();

            var fieldNameMedlemmer = "Medlemmer";

            var fieldNameAdvokater = "Kjernepartnere";
            var keyAdvokater = "Advokater";

            var fieldNameAssosierte = "Assosierte";
            var keyAssosierte = "Assosierte";

            var fieldNameDeltakere = "AndreDeltakendePartnere";
            var keyDeltakere = "Deltakere";

            var fieldNameAssistenter = "Assistenter";
            var keyAssistenter = "Assistenter";

            foreach (var dict in groupsAndMembersList)
            {
                if (dict.Keys.First().Equals(groupName))
                {
                    var users = new List<FieldUserValue>();
                    foreach (var userId in dict.Values.First())
                    {
                        if (userId != 0)
                        {
                            var user = new FieldUserValue { LookupId = userId };
                            users.Add(user);
                        }

                    }
                    item[fieldNameMedlemmer] = users;
                }
                if (dict.Keys.First().Contains(keyAdvokater))
                {
                    var users = new List<FieldUserValue>();
                    foreach (var userId in dict.Values.First())
                    {
                    ...
                    item[fieldNameAdvokater] = users;
                }
                else if (dict.Keys.First().Contains(keyAssosierte))
                {
                    var users = new List<FieldUserValue>();
                    foreach (var userId in dict.Values.First())
                    {
                    ...
                    item[fieldNameAssosierte] = users;
                }
                else if (dict.Keys.First().Contains(keyDeltakere))
                {
                    var users = new List<FieldUserValue>();
                    foreach (var userId in dict.Values.First())
                    {
                    ...
                    item[fieldNameDeltakere] = users;
                }
                else if (dict.Keys.First().Contains(keyAssistenter))
                {
                    var users = new List<FieldUserValue>();
                    foreach (var userId in dict.Values.First())
                    {
                    ...
                    item[fieldNameAssistenter] = users;
                }
        ...
        private static async Task PrintGroupMembers(ActiveDirectoryClient client, string groupName, int groupId,
            ClientContext ctx, string groupType)
        {
            Console.WriteLine("-------------------------*******************************-----------------");
            ...
            {
                foreach (var retrievedGroup in foundGroups)
                {
                    Console.WriteLine(retrievedGroup.DisplayName);
                    ...
                    if (retrievedGroup != null && retrievedGroup.ObjectId != null)
                    {
                        ...
                        try
                        {
                            IPagedCollection<IDirectoryObject> members =
                                await retrievedGroupFetcher.Members.ExecuteAsync();
                            ...
                            do
                            {
                                ...
                                foreach (IDirectoryObject member in directoryObjects)
                                {
                                    if (member is User)
                                    {
                                        ...
                                        //_groupsAndMembersList[groupName].SetValue();
                                        membersArr.Add(SPGetUserID((member as User).UserPrincipalName, ctx));
                                    }
                                    if (member is Group)
                                    {
                                        ...
                                        //Get members
                                        await
                                            PrintGroupMembers(client, (member as Group).DisplayName, groupId, ctx,
                                                "Undergruppe");
                                    }
                                }
                            ...
                            dict.Add(retrievedGroup.DisplayName, membersArr.ToArray());
                            _groupsAndMembersList.Add(dict);
        ...
        private static int SPGetUserID(string userPrincipalName, ClientContext ctx)
        {
            try
            {
                var user = ctx.Web.EnsureUser(userPrincipalName);
                ctx.Load(user);
                ctx.ExecuteQueryRetry();
                return user.Id;
            }
            ...
            IActiveDirectoryClient client, Application application)
        {
            ...
            {
                ExtensionProperty bevillingProperty = new ExtensionProperty
                {
                    Name = "Bevilling",
                    DataType = "Boolean",
                    TargetObjects = { "User" }
                ...
                try
                {
                    ...
                    application.ExtensionProperties.Add(bevillingProperty);
                    await application.UpdateAsync();
                    application.GetContext().SaveChanges();
                    Console.WriteLine("\nUser object extended successfully with extension: {0}.", bevillingProperty.Name);
                }
                catch (Exception e)
                {
                    Program.WriteError("\nError extending the user object {0}", Program.ExtractErrorMessage(e));
                }
                return bevillingProperty;
        ...
        private static async Task SetExtensionProperty(ExtensionProperty extension, User user)
        {
            ...
            try
            {
                if (extension != null && user != null && user.ObjectId != null)
                {
                    user.SetExtendedProperty(extension.Name, true);
                    await user.UpdateAsync();
